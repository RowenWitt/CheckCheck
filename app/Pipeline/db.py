...
from sqlalchemy import (
    create_engine, select, insert, update, func, inspect, and_)
...
# Will need a function to weave together S3 and DB data to feed to wave_dance
# Memory will be a concern, build with batching in mind,
# do some benchmarking to see what batch size is reasonable
# What is the size of an unpacked grib file in active memory?
# Ideally I can package data at least in sets of days... a week would be great
# A week of GRIB data would be 4 * 7 = 28 GRIB files worth of data,
# vague recollection says each file is ~7 MBs, 28 * 7 is 196 MBs, half weeks?
# The size of the data that can be represented will determine
# the size of the model batches/epochs, ideally the biggest possible epoch

# Because data will be weaved together here and passed directly
# to the wave_dance file, Fourier Analysis should occur here

# Considering two files, read_DB and write_DB for loaded memory optimization

# Fourier Fill needs to have a specific application
# for each required use, BUOY, TIDE, SAT_DAT, SURFLINE
# Should somehow weight available neighboring datapoints
...
class Database(object):
    ...
    def insert_weather(self, data: List[Dict]):
        ...
        accepts output of get_weather_update(correct_grid_calls)
        from buoy_tide_weather.py as input
        """
        ...
        # Get data WEATHER table from specific time range
        # for location (with default period)
    ...
    def insert_buoy(self, data: List[Dict]):
        ...
        # Call get new data function (list of locations and URLs, data.py)
        # from buoy_tide_weather.py
        ...
        # Get data BUOY table from specific time range
        # for location (with default period)
    ...
    def insert_tide(self, data: List[Dict]):
        ...
        # Get all data TIDE table from specific time range (default period)

        # Get all data TIDE table from specific time range
        # for location (with default period)
    ...
    # SURFLINE functions
        ...
        # Get all data SURFLINE table from specific time range (default period)

        # Get all data SURFLINE table from specific time range
        # for location (with default period)
    ...
    # SAT_S3 functions
        ...
        # Call get new data function (list of locations
        # and URLs, data.py, build URL possibility) from sat_dat.py

        # Insert data to S3 Bucket (possibilty to rename) from sat_dat.py

        # Get data from S3 Bucket
        # (should be able to deterministically reference files) from sat_dat.py

        # Fourier Fill (cool name right) for list of GRIB files, return FF
    ...
    def initialize_buoy(self):
        ...
        if insp.has_table('buoy_data') is False:
    ...
    def initialize_tide(self):
        ...
        insp = inspect(self.engine)
        if insp.has_table('tide_vals') is False:
    ...
    def initialize_weather(self):
        ...
        insp = inspect(self.engine)
        if insp.has_table('noaa_hourly_forecast') is False:
