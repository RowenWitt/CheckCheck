...
from app.Pipeline.data import lat_long, buoy_list
import json
import os
import requests
import datetime
...
class BTW(object):
    ...
    def get_historical_tide() -> List[List]:
        """
        Makes calls for each buoy on list for year intervals on list.
        Stores in one big array.  Only about 130k records
        so one big array is fine.  Not sure point at which that needs to
        be broken up.
        """
        ...
        for j in range(len(station_list_keys)):
            ...
            for i in range(len(keyList)):
                ...
                if (i + 1) % 5 == 0:
                    print(
                        "Buoy {} call {} / {} complete, sleeping 45".format(
                            id, i + 1, len(keyList)))
            time.sleep(45)

        return bigList

        def get_updated_buoy_data(input: Tuple) -> List:
            """
            gets updated buoy data for all buoys
            in tuple (id, lat/long) input
            """
            output_list = []
            for j in input:
                # remove slice for prod
                a = requests.get(
                    'https://www.ndbc.noaa.gov/data/realtime2/{}.txt'.format(
                        j[0]))
        ...
        def get_buoy_links() -> List:
            ...
            for a, b in lat_long:
                first_header = 'https://api.weather.gov/points/{},{}'.format(
                    a, b)
        ...
        def get_weather_update(calls) -> List:
            output_list = []
            slumb = 0
            for j in calls:
                slumb += 1
                ...
                for i in a:
                    try:
                        bid = j[1]
                        tar = i['startTime'][:i['startTime'].rindex(
                            '-')].replace('T', ' ')
                        dt = datetime.datetime.strptime(
                            tar, "%Y-%m-%d %H:%M:%S")
                        te = i['temperature']
